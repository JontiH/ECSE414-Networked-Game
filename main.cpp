...
#define PORT "3490" //Port to which the server and client  sockets listen to.
#define IP_DESTINATION "132.206.54.90" //IP where the file ServerV2.cpp is founf
#define MY_IP "192.168.82.132" //IP of the player
UDPSystem network = UDPSystem(IP_DESTINATION,PORT);
UDPSystem  listener = UDPSystem(IP_DESTINATION,PORT); //listens to packet from the server
UDPSystem  listener2 = UDPSystem(IP_DESTINATION,PORT); //listens to packet from the server
...
//tells the client if he is player 1 or player 2
char* connect_server(){
	   
    //wait to receive a packet which tells which player the client is
    network.connect2("Connecting to server\n");
    char * buff ;
    int len = 0;
    listener.init();
    printf("Player is waiting for a packet from the server\n");
    buff = listener.recvPacket(SEND_TIMEOUT,0);
    len = strlen(buff);
    printf("Player received a packet: packet contains \"%s\"\n",buff);
...
//sends  a packet to the server whose IP address is IP_DESTINATION
void sendPacket(char * message){
    network.connect2(message); //sends a packet to the server
...
void receivePacket(){
    ...
    int len = 0;
    listener2.init();
    printf("Player is waiting for a packet from the server port %s\n", PORT);
    buff = listener2.recvPacket(SEND_TIMEOUT,0);
    len = strlen(buff);
    printf("Player received a packet: packet contains \"%s\"\n",buff);
    listener2.closeSocket();
...
int main()
{
    char * player_no = connect_server(); //This line tells which player the client is
    printf("I am  %s\n", player_no);
    // setup window
    ...
    sideText.setColor(sf::Color(255, 0, 0));
    // set the text style
    ...
    while (window.isOpen())
    {
        ...
        if (victory == 0) {
            ...
            endText.setColor(sf::Color(255, 0, 0));
            // set the text style
